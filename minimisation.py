...
def simplex_minimization():
    num_variables = int(input("Enter the number of variables: "))
    num_constraints = int(input("Enter the number of constraints: "))

    c_min = list(map(float, input("Enter coefficients of the objective function to minimize: ").split()))
    A_min = get_constraint_coefficients(num_constraints, num_variables)
    b_min = list(map(float, input("Enter the right-hand side of the constraints: ").split()))

    m, n = len(A_min), len(c_min)
    tableau = np.zeros((m + 1, m + n + 1))

    tableau[:-1, :n] = np.array(A_min)
    tableau[:-1, -1] = b_min
    tableau[-1, :n] = np.array(c_min)

    slack_vars = np.eye(m)
    tableau[:-1, n:n+m] = slack_vars

    basic_vars = np.arange(n, n + m)
    non_basic_vars = np.arange(0, n)

    while np.any(tableau[-1, :n] > 0):
        pivot_col = np.argmax(tableau[-1, :n])
